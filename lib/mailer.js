...
async function sendReservationConfirmation({ email, name, reservation, puppy }) {
  ...
  await sendMail({
    to: email,
    subject: `Confirmation de réservation ${reservation.reservationNumber} — PomParadies GmbH`,
    html,
    text: [
      `CONFIRMATION DE RÉSERVATION — PomParadies GmbH`,
      ``,
      `Bonjour ${name},`,
      ``,
...
async function sendAdminNotification({ reservation, puppy }) {
  ...
  await sendMail({
    ...
    html,
    text: [
      `NOUVELLE RÉSERVATION — PomParadies GmbH`,
      ``,
...
async function sendStatusNotification({ email, name, reservationNumber, status, puppy }) {
  ...
  await sendMail({
    ...
    html,
    text: [
      `SUIVI DE RÉSERVATION — PomParadies GmbH`,
      ``,
      `Bonjour ${name},`,
      ``,
...
async function sendReplyToCustomer({ email, name, subject, message }) {
  ...
  await sendMail({
    to: email,
    subject: subject || 'PomParadies GmbH — Message de votre éleveur',
    html,
    text: [
      `MESSAGE — PomParadies GmbH`,
      ``,
      `Bonjour ${name},`,
      ``,
