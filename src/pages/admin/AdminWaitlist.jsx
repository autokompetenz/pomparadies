...
export default function AdminWaitlist() {
  ...
  const handleDelete = async (entry) => {
    if (!window.confirm(`Eintrag von ${entry.name || entry.email} löschen?`)) return;
    try {
  ...
  if (loading) return <div style={{ padding: 40 }}><Loader text="Warteliste wird geladen..." /></div>;
    ...
    <div style={{ padding: 'clamp(24px,5vw,48px) clamp(16px,4vw,44px) 60px', minHeight: '100vh', background: 'var(--bg)' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 900, fontSize: 'clamp(28px,4vw,48px)', color: 'var(--text)', letterSpacing: '-0.02em' }}>
          Warteliste <span style={{ color: 'var(--text-3)', fontSize: '0.55em', fontWeight: 600 }}>({entries.length})</span>
        </h1>
      </div>
      ...
      {entries.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px 24px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12 }}>
          ...
          <p style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>Keine Einträge</p>
          <p style={{ color: 'var(--text-3)' }}>Die Warteliste ist leer.</p>
        </div>
      ) : (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Name', 'E-Mail', 'Gewünschte Rasse', 'Eingetragen am', 'Aktion'].map(h => (
