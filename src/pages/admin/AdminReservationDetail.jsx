...
export default function AdminReservationDetail() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      addToast(data.message || 'Update durchgeführt', 'success');
      setComment('');
      load();
    } catch (err) {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      addToast('Nachricht an den Kunden gesendet', 'success');
      setReplyMsg('');
    } catch (err) {
      addToast(err.response?.data?.error || 'Sendefehler', 'error');
    } finally { setSendingReply(false); }
  };

  if (loading) return <div style={{ padding:40 }}><Loader /></div>;
  if (!reservation)  return <div style={{ padding:40, color:'var(--text-3)', fontSize:16 }}>Reservierung nicht gefunden.</div>;
  ...
  return (
    <div style={{ padding:'clamp(24px,5vw,48px) clamp(16px,4vw,44px) 60px', minHeight:'100vh', background:'var(--bg)' }}>
      <div style={{ display:'flex', alignItems:'center', gap:20, marginBottom:32, flexWrap:'wrap' }}>
        <div>
          <div className="section-eyebrow">Reservierung</div>
      ...
      <div className="admin-order-grid">
        <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
          <div style={cardStyle}>
            <p style={{ fontSize:11, fontWeight:800, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--primary)', marginBottom:18 }}>Kundeninformationen</p>
            <div className={isMobile ? 'admin-grid-2' : ''} style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
              <InfoRow label="Name" value={reservation.guestName} />
              <InfoRow label="E-Mail" value={reservation.guestEmail} />
              <InfoRow label="Telefon" value={reservation.guestPhone} />
              <InfoRow label="Beruf" value={reservation.guestProfession} />
              <InfoRow label="Heimadresse" value={reservation.guestHomeAddress} />
              <InfoRow label="Reserviert am" value={formatDate(reservation.createdAt)} />
              <InfoRow label="Zahlung" value={reservation.paymentLabel || (reservation.paymentMethod === 'full' ? 'Vollständig' : 'Anzahlung 50%')} />
              <InfoRow label="Haustier vorhanden?" value={reservation.hasPet === true ? 'Ja' : reservation.hasPet === false ? 'Nein' : '—'} />
              <InfoRow label="Schon ein Tier verloren?" value={reservation.hasLostPet === true ? 'Ja' : reservation.hasLostPet === false ? 'Nein' : '—'} />
            </div>
            {reservation.notes && (
              <div style={{ marginTop:16, padding:'12px 16px', background:'var(--primary-bg)', border:'1px solid var(--primary-border)', borderRadius:8 }}>
                <p style={{ fontSize:11, fontWeight:700, letterSpacing:'0.18em', textTransform:'uppercase', color:'var(--primary)', marginBottom:6 }}>Notizen</p>
          ...
          {reservation.puppy && (
            <div style={cardStyle}>
              <p style={{ fontSize:11, fontWeight:800, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--primary)', marginBottom:18 }}>Reservierter Welpe</p>
              ...
              <div className={isMobile ? 'admin-flex-wrap' : ''} style={{ display:'flex', gap:24, marginTop:16, paddingTop:16, borderTop:'1px solid var(--border)' }}>
                {reservation.discountAmount > 0 && <div><p style={{ fontSize:11, color:'var(--text-3)' }}>{'Rabatt'}</p><p style={{ fontWeight:700, color:'#22C55E', fontSize:16 }}>-{formatEuro(reservation.discountAmount)}</p></div>}
                <div><p style={{ fontSize:11, color:'var(--text-3)' }}>{'Anzahlung'}</p><p style={{ fontWeight:700, color:'var(--text)', fontSize:16 }}>{formatEuro(reservation.depositAmount || 0)}</p></div>
                {reservation.balanceAmount > 0 && <div><p style={{ fontSize:11, color:'var(--text-3)' }}>{'Restbetrag'}</p><p style={{ fontWeight:700, color:'var(--text)', fontSize:16 }}>{formatEuro(reservation.balanceAmount)}</p></div>}
                <div><p style={{ fontSize:11, color:'var(--text-3)' }}>{'Gesamt'}</p><p style={{ fontFamily:"'Outfit',sans-serif", fontWeight:900, fontSize:22, color:'var(--primary)' }}>{formatEuro(reservation.totalPrice || reservation.puppy?.price)}</p></div>
              </div>
            </div>
          )}
          ...
          {reservation.tracking?.length > 0 && (
            <div style={cardStyle}>
              <p style={{ fontSize:11, fontWeight:800, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--primary)', marginBottom:20 }}>Verlauf</p>
              <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
        ...
        <div>
          <form onSubmit={handleUpdate} style={{ ...cardStyle, position:'sticky', top:24 }}>
            <p style={{ fontSize:11, fontWeight:800, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--primary)', marginBottom:20 }}>Status aktualisieren</p>
            ...
            <div style={{ marginBottom:18 }}>
              ...
              <textarea value={comment} onChange={e => setComment(e.target.value)} rows={4}
                placeholder="Z. B. Ihr Welpe ist bereit..." className="input-luxury" style={{ resize:'none', fontSize:14 }} />
            </div>
          ...
          <div style={cardStyle}>
            <p style={{ fontSize:11, fontWeight:800, letterSpacing:'0.22em', textTransform:'uppercase', color:'var(--primary)', marginBottom:18 }}>Dem Kunden antworten</p>
            <div style={{ display:'flex', gap:10, marginBottom:16 }}>
              <a href={`mailto:${reservation.guestEmail}?subject=${encodeURIComponent('PomParadies GmbH — Reservierungsstatus ' + reservation.reservationNumber)}`}
            ...
            <textarea value={replyMsg} onChange={e => setReplyMsg(e.target.value)} rows={4}
              placeholder="Schreiben Sie dem Kunden eine Nachricht..." className="input-luxury" style={{ resize:'none', fontSize:14, marginBottom:14 }} />
            <button onClick={handleReply} disabled={sendingReply || !replyMsg.trim()} className="btn-primary" style={{ width:'100%', justifyContent:'center', padding:14, fontSize:14 }}>
              {sendingReply ? '⏳...' : '✉ Nachricht senden'}
            </button>
          </div>

          <div style={{ marginTop:20, padding:20, background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:12 }}>
            <p style={{ fontSize:11, fontWeight:800, letterSpacing:'0.15em', textTransform:'uppercase', color:'#DC2626', marginBottom:12 }}>Gefahrenzone</p>
            <p style={{ fontSize:13, color:'var(--text-3)', marginBottom:14, lineHeight:1.5 }}>
              Diese Reservierung endgültig löschen. Diese Aktion ist unumkehrbar.
            </p>
